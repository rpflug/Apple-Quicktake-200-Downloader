	...
	struct usb_serial_port *port, const unsigned char *buf, int count)
{
	...
	if (d_details->msg_format == msg_usa90) {
		maxDataLen = 64;
		dataOffset = 0;
	} else if (d_details->msg_format == msg_usa28 &&
		   (p_priv->cflag & PARENB)) {
		/* USA-28 parity mode uses a parity byte before every data byte. */
		maxDataLen = 31;
		dataOffset = 1;
	} else {
		maxDataLen = 63;
	...
	for (left = count; left > 0; left -= todo) {
		...
		out = this_urb->transfer_buffer + dataOffset;
		if (d_details->msg_format == msg_usa28 &&
		    (p_priv->cflag & PARENB)) {
			for (i = 0; i < todo; ++i) {
				unsigned char parity = hweight8(buf[i]) & 1;

				/* Odd parity is the inverse of the even parity bit. */
				if (p_priv->cflag & PARODD)
					parity ^= 1;
				out[2 * i] = parity ? TX_PARITY_BIT : 0;
				out[2 * i + 1] = buf[i];
			}
		} else {
			memcpy(out, buf, todo);
		...
		this_urb->transfer_buffer_length = dataOffset +
			((d_details->msg_format == msg_usa28 &&
			  (p_priv->cflag & PARENB)) ? 2 * todo : todo);
	...
	do {
		...
		if (urb->actual_length) {
			if (p_priv->cflag & PARENB) {
				/* Firmware returns DAT/PAR pairs in parity mode. */
				for (i = 0; i + 1 < urb->actual_length; i += 2) {
					unsigned char parity = hweight8(data[i]) & 1;
					int flag = TTY_NORMAL;

					if (p_priv->cflag & PARODD)
						parity ^= 1;
					if (!!(data[i + 1] & RX_PARITY_BIT) != parity)
						flag = TTY_PARITY;
					tty_insert_flip_char(&port->port, data[i], flag);
				}
			} else {
				    ...
				    int reset_port)
{
	...
	/* USA-28 firmware expects parity bytes in the bulk data streams. */
	msg.parity = !!(p_priv->cflag & PARENB);
